<?php
	...
	if($_SESSION["utente"]!=""){ //Controllo se è stato effettuato il login
		...
		if($check_classe_utente){ //Se l'array è pieno
			...
			$query ="SELECT  * FROM  `amici` WHERE amico_destinatario = ".$_SESSION["utente"]." AND stato =0"; //Query
			$n_richieste_ris =mysql_query($query); //Eseguo
			$n_richieste = mysql_num_rows($n_richieste_ris); //Conto le righe	
			$richieste = mysql_fetch_array($n_richieste_ris); //Creo l'array
				...
				<script type="text/javascript">
					...
					<div id="container">
						...
						<?php	while($richieste){
							...
							$query = "SELECT * FROM  `utenti` WHERE id_utente = ".$richieste["amico_mittente"].""; //Query
							$inforichiesta_ris = mysql_query($query); //Eseguo
							if($inforichiesta_ris>0) //Controllo se il risultato è positivo
								$inforichiesta = mysql_fetch_array($inforichiesta_ris); //Creo l'array ?>
							...
							<div id="box" class="bubble">
								...
								<form method= "post" action="accettar.php?idM=<?php echo $richieste["amico_mittente"]; ?>&idD=<?php echo $richieste["amico_destinatario"]; ?>">
									<input type="submit" id="acc" value="Accetta">
								</form>
								<form method= "post" action="rifiutar.php?idM=<?php echo $richieste["amico_mittente"]; ?>&idD=<?php echo $richieste["amico_destinatario"]; ?>">
									<input type="submit" id="rif" value="Rifiuta">
								</form>
							</div>
							<?php 
							$richieste = mysql_fetch_array($n_richieste_ris);
						} ?>					
