				...
				<script type="text/javascript">
					...
					<div id="container">
						...
						<?php 
							...
							$i=0;
							while($i<$dim){
								$query= "select * from utenti where id_utente='".$vettore[$i]."' " ; //Query
								$utente_trv_ris = mysql_query($query); //Eseguo
								if($utente_trv_ris>0) //Controllo se il risultato è positivo
									$utente_trv=mysql_fetch_array($utente_trv_ris); //Creo l'array ?>	
								...
								<div id="box" class="bubble">
									<div id="info_us"> 
										<a href="user.php?id=<?php echo $utente_trv["id_utente"]?>;"><?php echo $utente_trv["nome"]." ".$utente_trv["cognome"] ?></a>
									</div>
								</div>	
								<?php	$i++;
							}
